...
public class BrokerConnection implements Runnable {
	...
	private ArrayList<BrokerMessage> messageQueue;
	...
	public BrokerConnection(Socket clientSocket, MessageBroker parent) {
		...
		this.parent = parent;
		this.isOpen = true;
		this.messageQueue = new ArrayList<BrokerMessage>();
	}
		...
		try {		
			...
			while(isOpen) {
				try {
					...
					BrokerMessage receivedMessage = new BrokerMessage(latestMsg);
	 ...
	 */
	private void action(BrokerMessage receivedMessage){
	...
	public synchronized void enqueueMessage(BrokerMessage message)
	{
		if(message != null){
