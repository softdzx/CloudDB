...
public class KVEcs {
	...
	private ArrayList<NodeInfo> idleservers = new ArrayList<NodeInfo>();
	 ...
	 * call launches the server with the specified cache size and displacement
	 * strategy. You can assume that the KVServer.jar is located in the same
	 * directory as the ECS. All servers are initialized with the meta­data and
						...
						try {
							...
							if (ipport.length == 2) {
								...
								if (hashthreads.remove(hashedkey) != null) {
									final NodeInfo toremove = metadata.getServer(hashedkey);

									logger.info(
											"server with ip:" + ipport[0] + " and port:" + ipport[1] + " has failed");
		...
		try {
			...
			try {
				while ((line = reader.readLine()) != null) {
					...
					if (node.length == 3) {
						idleservers.add(new NodeInfo(node[1], node[2]));
					}
				}
				reader.close();
	...
	public TreeMap<String, NodeInfo> getServers() {
	...
	public void handleFailure(NodeInfo server) {
		idleservers.add(server);

		// don't need to move data if the number of servers is less than or
		// equal to 3
		if (metadata.size() > 3) {
			NodeInfo preServer = metadata.getPredecessor(server.hashedkey);

			NodeInfo sepreServer = metadata.getPredecessor(preServer.hashedkey);

			NodeInfo suceServer = metadata.getSuccessor(server.hashedkey);
			NodeInfo sesuceServer = metadata.getSuccessor(suceServer.hashedkey);
			NodeInfo thsuceServer = metadata.getSuccessor(sesuceServer.hashedkey);

			hashthreads.get(suceServer.hashedkey).receiveData();
			hashthreads.get(sesuceServer.hashedkey).receiveData();

			hashthreads.get(preServer.hashedkey).moveData(preServer.from, preServer.to, sesuceServer.ip,
					Integer.parseInt(sesuceServer.port) - 20);
			hashthreads.get(sepreServer.hashedkey).moveData(sepreServer.from, sepreServer.to, suceServer.ip,
					Integer.parseInt(suceServer.port) - 20);

			// thsuceServer and suceServer is same when size is 4
			if (metadata.size() != 4) {
				hashthreads.get(thsuceServer.hashedkey).receiveData();
				hashthreads.get(suceServer.hashedkey).moveData(suceServer.from, suceServer.to, thsuceServer.ip,
						Integer.parseInt(thsuceServer.port) - 20);
			}
		}
	...
	/*
	 * Create a new KVServer with the specified cache size and displacement
	 * strategy and add it to the storage service at an arbitrary position.
	 */
	public void addNode(final int newcacheSize, final String displacementStrategy) {
		...
		new Thread() {
			public void run() {
				...
				if (idleservers.size() != 0) {
					NodeInfo newworkingserver = getRandomNode();
					Runtime run = Runtime.getRuntime();

					Process proc;
					try {
						proc = run.exec("java -jar ./cdb-server.jar " + newworkingserver.port);
						procs.add(proc);
					...
					NodeInfo successor = metadata.getSuccessor(newworkingserver.hashedkey);

					try {
						Socket kvserver = null;
						while ((kvserver = ecsServer.accept()) != null) {
							// if(kvserver.getInetAddress().toString().equals("/"
							// + newworkingserver.ip) && kvserver.getPort() ==
							// Integer.parseInt(newworkingserver.port)){
							ServerConnection connection = new ServerConnection(kvserver, newcacheSize,
									displacementStrategy, metadata, logger);
					...
					suserver.moveData(newworkingserver.from, newworkingserver.to, newworkingserver.ip,
							Integer.parseInt(newworkingserver.port) - 20);

					if (metadata.getServers().size() > 3) {
						NodeInfo sesuserver = metadata.getSuccessor(successor.hashedkey);
						NodeInfo thsuserver = metadata.getSuccessor(sesuserver.hashedkey);
						hashthreads.get(thsuserver.hashedkey).removeData(newworkingserver.from, newworkingserver.to);

						NodeInfo preserver = metadata.getPredecessor(newworkingserver.hashedkey);
						NodeInfo sepreserver = metadata.getPredecessor(preserver.hashedkey);

						hashthreads.get(newworkingserver.hashedkey).receiveData();
						hashthreads.get(preserver.hashedkey).moveData(preserver.from, preserver.to, newworkingserver.ip,
								Integer.parseInt(newworkingserver.port) - 20);

						hashthreads.get(newworkingserver.hashedkey).receiveData();
						hashthreads.get(sepreserver.hashedkey).moveData(sepreserver.from, sepreserver.to,
								newworkingserver.ip, Integer.parseInt(newworkingserver.port) - 20);
	...
	public void removeNode() {
		Thread t = new Thread() {
			public void run() {
				NodeInfo toremove = metadata.getFirstServer();
	...
	public void startEcs(final int port, final int numberOfNodes, final int cacheSize,
			final String displacementStrategy) {
		new Thread() {
			public void run() {
				try {
					...
					while (currentNode != numberOfNodes && (kvserver = ecsServer.accept()) != null) {
						...
						ServerConnection connection = new ServerConnection(kvserver, cacheSize, displacementStrategy,
								metadata, logger);
	...
	private NodeInfo getRandomNode() {
		Random random = new Random();
		NodeInfo newworkingserver = idleservers.get(random.nextInt(idleservers.size()));

		logger.info("The server with ip:" + newworkingserver.ip + " and port:" + newworkingserver.port
				+ " is picked as a new node");

		// Recalculate and update the metadata of the storage service
	...
	private void startServers() {
		...
		for (NodeInfo server : metadata.getServers().values()) {
			try {
				proc = run.exec("java -jar ./cdb-server.jar " + server.port);
				procs.add(proc);
